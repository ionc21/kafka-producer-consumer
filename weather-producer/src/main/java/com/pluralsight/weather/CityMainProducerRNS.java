...
public class CityMainProducerRNS {

    private static final Logger LOG = LoggerFactory.getLogger(CityMainProducerRNS.class);
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        KafkaProducer<City, Main> producer = new KafkaProducer<>(props);
        ...
        while(true) {
                ...
                City key = City.newBuilder()
                                .setCity(CITY)
                                .build();
                Main value = buildAvroMain(currentWeather);

                LOG.info("Sending to Kafka on the " + WEATHER_TOPIC + " topic the following message: \n" + CITY + " : " + currentWeather);

                ProducerRecord<City, Main> producerRecord =
